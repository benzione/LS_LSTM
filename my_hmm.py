...
class MultinomialHMM(_BaseHMM):
    ...
    def __init__(self, n_components=1, n_features=1,
                 startprob_prior=1.0, transmat_prior=1.0,
                 algorithm="viterbi", random_state=None,
                 n_iter=10, tol=1e-2, verbose=False,
                 params="ste", init_params="ste"):
        _BaseHMM.__init__(self, n_components=n_components, n_features=n_features,
                          startprob_prior=startprob_prior,
    ...
    def _compute_log_likelihood(self, X):
        tmp = np.where(self.emissionprob_ == 0)
        self.emissionprob_[tmp] = 1.e-17
        self.emissionprob_ = self.emissionprob_ / np.sum(self.emissionprob_)
    ...
    def _do_mstep(self, stats):
        ...
        if 'e' in self.params:
            tmp1 = stats['obs'].sum(1)[:, np.newaxis]
