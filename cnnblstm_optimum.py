...
def run_model(X_train, y_train, X_validate, y_validate, parameters, str_file1):
    ...
    model.add(Bidirectional(LSTM(int(parameters[6]))))
    ...
    np.savetxt('./Data/Results_all/cnnblstm_%s.csv' % (str_file1), predicted, delimiter=",")
    # conf = confusion_matrix(y_validate, predicted)
    # np.save('./Data/Results_all/cnnblstm_%s.npy' % str_file1, conf)
...
def main():
    ...
    results = np.load('./Data/Results_RNN/cnnblstm_results.npy')
    max_results = np.argmax(results)
    parameters = np.load('./Data/Results_RNN/cnnblstm_parameters_cv%d.npy' % max_results)
    ...
    for i, file_tmp in enumerate(os.listdir("./Data/Trajectories")):
        ...
        print('run test cnnblstm %s %d' % (str_file1, i))
        run_test(trajectories_traininig, trajectories_testing, ls_training, ls_testing, parameters, str_file1)
    # np.save('./Data/Results_RNN/cnnblstm_results_users.npy', results_cv)
