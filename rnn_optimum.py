...
def run_model(X_train, y_train, X_validate, y_validate, parameters, str_file1):
    ...
    model.add(SimpleRNN(int(parameters[2])))
    model.add(Dropout(float(parameters[3])))
    model.add(Dense(k2, activation='softmax'))
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.fit(X_train, to_categorical(y_train), epochs=int(parameters[4]), batch_size=int(parameters[5]), verbose=0,
              shuffle=True)
    ...
    np.savetxt('./Data/Results_all/rnn_%s.csv' % (str_file1), predicted, delimiter=",")
    # conf = confusion_matrix(y_validate, predicted)
    # np.save('./Data/Results_all/rnn_%s.npy' % str_file1, conf)
...
def main():
    ...
    results = np.load('./Data/Results_RNN/rnn_results.npy')
    max_results = np.argmax(results)
    parameters = np.load('./Data/Results_RNN/rnn_parameters_cv%d.npy' % max_results)
