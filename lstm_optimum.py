...
def run_model(X_train, y_train, X_validate, y_validate, parameters, str_file1):
    ...
    np.savetxt('./Data/Results_all/lstm_%s.csv' % (str_file1), predicted, delimiter=",")
    # conf = confusion_matrix(y_validate, predicted)
    # np.save('./Data/Results_all/lstm_%s.npy' % str_file1, conf)
...
def main():
    ...
    results = np.load('./Data/Results_RNN/lstm_results.npy')
    max_results = np.argmax(results)
    parameters = np.load('./Data/Results_RNN/lstm2_parameters_cv%d.npy' % max_results)
