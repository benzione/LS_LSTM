...
def run_model(X_train, y_train, X_validate, y_validate, parameters, str_file1):
    ...
    model.add(Bidirectional(LSTM(int(parameters[2]))))
    ...
    np.savetxt('./Data/Results_all/blstm_%s.csv' % (str_file1), predicted, delimiter=",")
    # conf = confusion_matrix(y_validate, predicted)
    # np.save('./Data/Results_all/blstm_%s.npy' % str_file1, conf)
...
def main():
    ...
    # np.save('./Data/Results_RNN/blstm_results_users.npy', results_cv)
